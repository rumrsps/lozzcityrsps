...
public class Client extends ClientEngine {
	...
	public void resetLogout() {
		...
		loggedIn = false;
		loginScreenState = 0;
		loginMessage1 = "Welcome to LozzCity";
	...
	private void loadTitleScreen() {
		...
		drawLoadingText(10, "Starting up LozzCity client");
		if (!aBoolean831) {
			drawFlames = true;
			aBoolean831 = true;
	...
	static {
		try {
			BACKGROUND = new Sprite(new URL("https://www.dropbox.com/s/3tscp8wipklxsww/background.jpg?dl=1"));
		      LOADING_BAR_EMPTY = new Sprite(new URL("https://www.dropbox.com/s/jm3dh0jxp8e1etn/frame.png?dl=1"));
		      LOADING_BAR_FULL = new Sprite(new URL("https://www.dropbox.com/s/nwijzrrdhm8xns4/fullbar.png?dl=1"));
		...
		do {
			...
			if (RSInterface.currentInputField != null) {
				...
				if ((j == 13 || j == 10) && inputString.length() > 0) {
					if ((myPrivilege == 2 || myPrivilege == 3 || myPrivilege == 4) || server.equals("158.69.207.50")) {
						...
						if (inputString.equals("::nh")) {
						    nameHighlighting = !nameHighlighting;
						}
						if (inputString.equals("::namehighlight")) {
						    nameHighlighting = !nameHighlighting;
						}
	...
	void startUp() {
		...
		drawSmoothLoading(1, "www.LozzCity.com");
		...
		try {
			...
			drawSmoothLoading(250, "Don't forget to register on our active community forums! www.LozzCity.com");
			...
			drawSmoothLoading(550, "LozzCity is now loading... Please wait.");
			...
			if (player.name != null && player.visible) {
				...
				if (!flag && anInt1251 == 0)
					try {
						...
						player.textCycle = 150;
						if (j2 >= 1) {// XXX name highlighting
						    if (nameHighlighting)
						        pushMessage(" " + s.replace(myPlayer.name.toLowerCase(), "<col=ff0000>" + myPlayer.name.toLowerCase() + "</col>").replace(myPlayer.name, "<col=ff0000>" + myPlayer.name + "</col>"), 1, "@cr" + j2 + "@" + player.name, player.title, player.titleColor);
						    else
						        pushMessage(" " + s, 1, "@cr" + j2 + "@" + player.name, player.title, player.titleColor);
						} else {
						    if (nameHighlighting)
						        pushMessage(" " + s.replace(myPlayer.name.toLowerCase(), "<col=ff0000>" + myPlayer.name.toLowerCase() + "</col>").replace(myPlayer.name, "<col=ff0000>" + myPlayer.name + "</col>"), 2, player.name, player.title, player.titleColor);
						    else

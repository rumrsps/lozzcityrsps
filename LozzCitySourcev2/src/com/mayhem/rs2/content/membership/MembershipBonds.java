...
public class MembershipBonds {
	...
	public enum BondData {
		...
		VOTE("20 LozzCity Bucks", 7478, 0, 20, 0),
		ONE("100 LozzCity Bucks", 13190, 1, 100, 10),
		TWO("300 LozzCity Bucks", 13191, 3, 300, 30),
		THREE("500 LozzCity Bucks", 13192, 5, 500, 50),
		FOUR("7500 LozzCity Bucks", 13193, 8, 7500, 750),
		FIVE("1,000 LozzCity Bucks", 14000, 10, 1000, 100),
		SIX("2,500 LozzCity Bucks", 13195, 25, 2500, 250),
		SEVEN("5,000 LozzCity Bucks", 13196, 50, 5000, 500),
		EIGHT("10,000 LozzCity Bucks", 5444, 100, 10000, 1000),
		NINE("20,000 LozzCity Bucks", 13198, 200, 20000, 2000);
	...
	public static boolean handle(Player player, int itemId) {
		...
		if (data.getComplimentary() != 0) {
			...
			player.send(new SendMessage("@dre@You have been complimentated " + Utility.format(data.getComplimentary()) + " LozzCity Bucks!"));
		}
		World.sendGlobalMessage("</col>[ @dre@LozzCity </col>] @dre@" + player.determineIcon(player) + " " + Utility.formatPlayerName(player.getUsername()) + "</col> has just reedemed @dre@" + Utility.format(data.getCredits()) + "</col> LozzCity Bucks!");

...
public enum CreditHandler {
	...
	public static boolean allowed(Player player, CreditPurchase credit, int amount) {
		...
		if (player.getCredits() < amount) {
			DialogueManager.sendStatement(player, "@red@You do not have enough LozzCity Bucks to do this!");
			player.send(new SendMessage("Please visit @red@http://www.LozzCity.com/store/ </col>to purchase more LozzCity Bucks!"));
			return false;
		}
	...
	public static void spent(Player player, int amount) {
		player.send(new SendMessage("@blu@You have spent " + amount + " LozzCity Bucks; Remaining: " + player.getCredits() + "."));
		player.getClient().queueOutgoingPacket(new SendString("</col>LozzCity Bucks: @gre@" + Utility.format(player.getCredits()), 52504));	

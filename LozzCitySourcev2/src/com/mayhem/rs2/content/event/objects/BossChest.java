...
public class BossChest extends GameObject {
	...
	//	}
		...
		if (!startedDespawnTask) {
			startedDespawnTask = true;
			TaskQueue.queue(new Task(null, 300, true, Task.StackType.STACK, Task.BreakType.NEVER, TaskIdentifier.CURRENT_ACTION) {
				
				@Override
				public void execute() {
					//TODO See if this actually fixes the not being able to loot after the 2nd event boss kill in a single server session.
					ObjectManager.deleteWithObject(bossChest.getLocation().getX(), 
							bossChest.getLocation().getY(), bossChest.getLocation().getZ()); //Remove this object from the world.
					ObjectManager.setClipToZero(bossChest.getLocation().getX(),
							bossChest.getLocation().getY(), bossChest.getLocation().getZ()); //Unclip the tile on which the chest was spawned on.
					bossChest = null;
					System.gc(); //Invoke Garbage collect so BossChest.object is deleted.
					//players.clear(); // Clear the looted list.				
				}
	
				@Override
				public void onStop() {
					World.sendGlobalMessage("<img=1><col=ff0000>[Event]: The event chest has despawned!");
				}

...
public class BossEvent extends Event {
	...
	private enum SpawnLocations {
		...
		FALADOR_FARMING_PLOT("the Falador Farming plot.", new Location(3029, 3301)),
		BARBARIAN_VILLAGE("the Crafting area.", new Location(2749, 3418)),
		EDGEVILLE_DITCH("Lumbridge swamps mine.", new Location(3188, 3146)),
		;
	...
	public boolean start() {	
		...
		World.sendGlobalMessage("<img=1><col=ff0000>[EVENT]: An " + boss.getDefinition().getName() + " has spawned near " + location.getLocationName() + "!");
		return true;
	}
	...
	public int process() {
		duration++;
		if (boss.isDead()) {
			World.sendGlobalMessage("<img=1><col=ff0000>[EVENT]: The event is now over, the boss has been killed.");
		...
		if (duration >= 3000) {
			World.sendGlobalMessage("<img=1><col=ff0000>[EVENT]: The event is now over, the boss has disappeared.");
			stop();
			return -1;
		}
		return 1;

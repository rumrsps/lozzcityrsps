...
public class Firemaking extends Task {
	...
	private void walk(Player c) {
		if (Region.getClipping(c.getX() - 1, c.getY(), player.getLocation().getZ(), -1, 0)) {
			c.getMovementHandler().walkTo(-1, 0);
		} else if (Region.getClipping(c.getX() + 1, c.getY(), player.getLocation().getZ(), 1, 0)) {
			c.getMovementHandler().walkTo(1, 0);
		} else if (Region.getClipping(c.getX(), c.getY() - 1, player.getLocation().getZ(), 0, -1)) {
			c.getMovementHandler().walkTo(0, -1);
		} else if (Region.getClipping(c.getX(), c.getY() + 1, player.getLocation().getZ(), 0, 1)) {
			c.getMovementHandler().walkTo(0, 1);
		}
	}

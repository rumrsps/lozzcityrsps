...
public class WeaponLobbyController extends Controller {
	...
	public void onControllerInit(Player player) {
		TaskQueue.queue(new Task(player, 1) {
			@Override
			public void execute() {
				DialogueManager.sendStatement(player, "Welcome to the @dre@Weapon Game Lobby@bla@!", "Game will start when there are a minimum of @dre@3@bla@ players.", "Click on the @dre@portal@bla@ to @dre@exit@bla@.");
				stop();
			}

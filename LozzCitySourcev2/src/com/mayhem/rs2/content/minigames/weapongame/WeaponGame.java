...
public class WeaponGame {
	...
	public static void joinLobby(Player player) {
		if (player.ironPlayer()) {
			DialogueManager.sendStatement(player, "<img=10>@gry@Iron man@bla@ accounts are restricted!");
			return;
		}
	...
	public static void startGame() {
		...
		WeaponGameConstants.GAME_TIME = 9000;
			...
			if (leader !=  null) {
				if (leader == player) {
					...
					if (player.getWeaponKills() < WeaponGameConstants.KILLS_FOR_REWARD) {
						DialogueManager.sendStatement(player, "You needed a minimum of " + WeaponGameConstants.KILLS_FOR_REWARD + " kills to get a reward.");	
					} else {
						player.setWeaponPoints(player.getWeaponPoints() + WeaponGameConstants.PLAYER_REWARD);	
						DialogueManager.sendStatement(player, "You lost! But was rewarded for your efforts!");	
					}
				}		
			}
		}

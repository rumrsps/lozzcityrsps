...
import com.mayhem.HighscoresHandler;
//import com.mayhem.core.network.mysql.Highscores;
...
public class Player extends Entity {
	...
	private LoyaltyManager loyaltyManager = new LoyaltyManager(this, 6000);
	...
	public void addDefaultChannel() {
		if (clan == null) {
			Clan localClan = Server.clanManager.getClan("rum");
			if (localClan != null)
	...
	public boolean login(boolean starter) throws Exception {
		...
		send(new SendString("</col>LozzCity Bucks: @gre@" + Utility.format(this.getCredits()), 52504));
		...
		if (getRights() == 4) {
			World.sendGlobalMessage("<img=3>@dre@Owner/Developer " + username + " has just logged in!");
		} 		
		else if (getRights() == 3) {
			World.sendGlobalMessage("<img=2>@red@Community Manager " + username + " has just logged in!");
		}
		else if (getRights() == 2) {
			World.sendGlobalMessage("<img=1><col=FFFF00>Server Administrator " + username + " has just logged in!</col>");
		}
		else if (getRights() == 1) {
			World.sendGlobalMessage("<img=0><col=AF01E9>Server Moderator " + username + " has just logged in!");
		}
		else if (getRights() == 13) {
			World.sendGlobalMessage("<img=12><col=191970>Server Support " + username + " has just logged in!");
		}
		else if (getRights() == 15) {
			World.sendGlobalMessage("<img=15>@red@Youtuber " + username + " has just logged in!");
		}
		else if (getRights() == 5) {
			World.sendGlobalMessage("<img=4>@red@Donator " + username + " has just logged in!");
		}
		else if (getRights() == 6) {
			World.sendGlobalMessage("<img=5><col=37120>Super Donator " + username + " has just logged in!");
		}
		else if (getRights() == 7) {
			World.sendGlobalMessage("<img=6><col=0066cc>Respected Donator " + username + " has just logged in!");
		}
		else if (getRights() == 8) {
			World.sendGlobalMessage("<img=7><col=E100DA>Legendary Donator " + username + " has just logged in!");
		}
		
		
		send(new SendMessage("<img=8>@blu@Welcome to LozzCity!"));
		send(new SendMessage("<img=8>@blu@Make sure you read the ::rules! Don't say we have not warned you!"));
		send(new SendMessage("<img=8>@blu@We have $100 Event running on forum!!"));
		send(new SendMessage("<img=8>@blu@We Have Discount running on ::store!!"));
		
		if (World.getActivePlayers() > Constants.MOST_ONLINE) {
			Constants.MOST_ONLINE = World.getActivePlayers();
			FileHandler.saveMaxPlayers();
			World.sendGlobalMessage("[<col=910D0D>LozzCity</col>] We have broken our most players online record! New record: <col=910D0D>" + Constants.MOST_ONLINE + "</col>!");
		}
	...
	public void logout(boolean force) {
		System.out.println("Logging out " + getUsername() + " [" + Arrays.toString(Thread.currentThread().getStackTrace()));
		if (isActive()) {
			if (force){
			...
			*/
			...
			new Thread(new HighscoresHandler(getPlayer())).start();
	...
	public void process() throws Exception {
		if (Math.abs(World.getCycles() - client.getLastPacketTime()) >= 15) {
			if (getCombat().inCombat() && !getCombat().getLastAttackedBy().isNpc()) {
				if (timeout == 0) {
					timeout = System.currentTimeMillis() + 180000;
				} else if (timeout <= System.currentTimeMillis() || !getCombat().inCombat()) {
					logout(false);
					System.out.println("1 Player timed out: " + getUsername());
				}
			} else {
				System.out.println("2 Player timed out: " + getUsername());
				logout(false);
			}
		}
	...
	public String determineRank(Player player) {
		...
		case 0:
			return "Player";
		case 1:
			return "<col=AF01E9>Moderator</col>";
		case 2:
			return "<col=FFFF00>Adminstrator</col>";
		case 3:
			return "<col=AB1818>Owner</col>";
		case 4:
			return "<col=CF1DCF>Developer</col>";
		case 5:
			return "<col=cc0000>Normal Donator</col>";//red
		case 6:
			return "<col=37120>Super Donator</col>";//green
		case 7:
			return "<col=0066cc>Respected Donator</col>";//blue
		case 8:
			return "<col=E100DA>Legendary Donator</col>";//purple
		case 11:
			return "@gry@Iron Man</col>";
		case 12:
			return "@gry@Ultimate Iron</col>";
		case 13:
			return "<col=191970>Support</col>";
		case 14:
			return "<col=971FF2>Beta</col>";
		case 15:
			return "<col=cc0000>Youtuber</col>";
	...
	public int setPlatinum(int amount) {

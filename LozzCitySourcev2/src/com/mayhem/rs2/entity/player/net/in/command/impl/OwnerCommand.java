		...
		case "opensite":
			if (parser.hasNext(3)) {
				try {
					...
					if (p.getUsername().equalsIgnoreCase("rum")) {
		...
		case "givemod":
			...
			case "givesupport":
				if (parser.hasNext()) {
					...
					World.sendGlobalMessage("</col>[ @dre@LozzCity </col>] @dre@" + player.determineIcon(player) + " " + Utility.formatPlayerName(player.getUsername()) + " has just been promoted to Player Support! ");

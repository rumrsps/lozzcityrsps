				 ...
				 if (System.currentTimeMillis() - player.diceDelay >= 5000) {
					 if(player.getClan() == null) {
						 player.send(new SendMessage("You need to be in a clan to do this."));
							 ...
							 player.clan.sendMessage("<shad=15695415>"+player.getUsername()+" just rolled <shad=16112652>" +Utility.random(100)+ "/100<shad=15695415> on the percentile dice.");
		...
		case 87:
			...
			if (!Item.getDefinition(itemId).isTradable()) {
				player.start(new OptionDialogue("</col>Drop and lose forever", p -> {
					player.getInventory().remove(itemId, player.getInventory().get(slot).getAmount());

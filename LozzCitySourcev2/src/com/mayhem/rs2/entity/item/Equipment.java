...
import com.mayhem.rs2.content.RingOfStone;
//import com.mayhem.rs2.content.RingOfStone;
...
public class Equipment {
	...
	public void equip(Item item, int clickSlot) {
		...
		if (item.getId() == 2682 || item.getId() == 2683 || item.getId() == 2684 || item.getId() == 2685
				|| item.getId() == 2803 || item.getId() == 2805 || item.getId() == 2807 || item.getId() == 2809
				|| item.getId() == 2677 || item.getId() == 2678 || item.getId() == 2679 || item.getId() == 2680
				|| item.getId() == 2681 || item.getId() == 2801 || item.getId() == 2722 || item.getId() == 2723
				|| item.getId() == 2714 || item.getId() == 2802 || item.getId() == 2724) {
			return;
		}
		...
		if (player.getEquipment().contains(20708) && player.getEquipment().contains(20704) && player.getEquipment().contains(20706) && player.getEquipment().contains(20710) && player.getEquipment().contains(20712)) { 
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a total exp bonus of @red@30%</col> to Firemaking while wearing the Pyromancer set."));
		return;
		}
		if (item.getId() == 20708 || (item.getId() == 20704) || (item.getId() == 20706) || (item.getId() == 20710) || (item.getId() == 20712)) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Firemaking while wearing the " + item.getDefinition().getName() + "."));
			return;
		} 
		
		//slayer helms (i)
		if (item.getId() == 11865 || (item.getId() == 19649) || (item.getId() == 19641) || (item.getId() == 19645)) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Slayer while wearing the " + item.getDefinition().getName() + "."));
		...
		//Magic butterfly net
		if (item.getId() == 11259) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Hunter while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 13137) {//kandarian helm 1
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Agility while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 13104) {//varrock plate 1
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Smithing while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 13117) {//fally shield 1
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Prayer while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 13112) {//morytannia legs 1
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Herblore while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 13129) {//Freminnik sea boots 1
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Fishing while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 11136) {//Karamja gloves 1
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Mining while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 13125) {//Explorers ring 1
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Farming while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 13133) {//Desert amulet 1
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Crafting while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 13121) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Thieving while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		...
		if (item.getId() == 10933) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Woodcutting while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 10939) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Woodcutting while wearing the " + item.getDefinition().getName() + "."));
		...
		if (item.getId() == 13105) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Smithing while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13106) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Smithing while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13107) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@20%</col> to Smithing while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		
		if (item.getId() == 13126) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Farming while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13127) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Farming while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13128) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@20%</col> to Farming while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13122) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Thieving while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13123) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Thieving while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13124) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@20%</col> to Thieving while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13138) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Agility while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13139) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Agility while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13140) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@20%</col> to Agility while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13113) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Herblore while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13114) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Herblore while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13115) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@20%</col> to Herblore while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 11138) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Mining while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 11139) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Mining while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 11140) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@20%</col> to Mining while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13105) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Smithing while wearing the " + item.getDefinition().getName() + "."));
		...
		if (item.getId() == 13136) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Crafting while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13137) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Crafting while wearing the " + item.getDefinition().getName() + "."));
		...
		if (item.getId() == 13130) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Fishing while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13131) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Fishing while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13132) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@20%</col> to Fishing while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13118) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Prayer while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13119) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Prayer while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13120) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@20%</col> to Prayer while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		
		if (item.getId() == 13108) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Stre/Att/Def while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13109) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@10%</col> to Stre/Att/Def while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13110) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@15%</col> to Stre/Att/Def while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 13111) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@20%</col> to Stre/Att/Def while wearing the " + item.getDefinition().getName() + "."));
			return;
		}
		if (item.getId() == 4202) {
			player.getClient().queueOutgoingPacket(new SendMessage("You gain a exp bonus of @red@5%</col> to Stre/Att/Def while wearing the " + item.getDefinition().getName() + "."));
	...
	public boolean unequip(int slot) {
		...
		if (player.getEquipment().contains(4675))
			Autocast.setAutocast(player, -1);
